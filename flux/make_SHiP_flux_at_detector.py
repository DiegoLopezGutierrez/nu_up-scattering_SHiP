...
FLUX_FILE = 'neutrinos_detector.root'
...
histogram_vtau     = root_file.Get("hnu_tau")
histogram_vtaubar  = root_file.Get("hnu_tau_bar")

# Calculate overall normalization
NORMALIZATION_POT = 5e13        # NPOT used for calculating this flux
NORMALIZATION_AREA = 0.40*0.40  # detector area used for calculating this flux
...
for i in range(1, histogram_vmu.GetNbinsX() + 1):
    ...
    # bin contents (neutrino count normalized to 1 POT and 1 m^2 detector area)
    ...
    bin_contents_vtau.append( histogram_vtau.GetBinContent(i) / NORMALIZATION)
    ...
    bin_contents_vtaubar.append( histogram_vtaubar.GetBinContent(i) / NORMALIZATION)
...
# calculate total neutrino count per POT per cm^2 for future test
...
# get the bin edges and centers for the linear (old) and logarithmic (new) bins
...
save_flux(rebinned_counts,log_bins,'normalized_flux_at_detector.csv',top_row=top_row)
...
top_row = ['Elow [GeV]',
           ' Ehigh [GeV]',
           ' numu [GeV^-1 POT^-1 m^-2]',
           ' numubar [GeV^-1 POT^-1 m^-2]',
           ' nue [GeV^-1 POT^-1 m^-2]',
           ' nuebar [GeV^-1 POT^-1 m^-2]',
           ' nutau [GeV^-1 POT^-1 m^-2]',
           ' nutaubar [GeV^-1 POT^-1 m^-2]']

save_flux(rebinned_diff_counts,log_bins,'normalized_diff_flux_at_detector.csv',top_row=top_row)

# plot SHiP neutrino flux at the detector (assumed to be 25 m away from proton target)
...
fig.savefig("../plots/flux_SHiP_at_detector.pdf", dpi=100)
...
ax2.set_title(r'{\bf At detector}')
ax2.set_xscale('log')
ax2.set_yscale('log')
...
fig2.savefig("../plots/diff_flux_SHiP_at_detector.pdf", dpi=100)